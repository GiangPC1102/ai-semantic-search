...
class LLM:
    ...
    def complete_json(
        self, system_prompt: str, user_payload: dict, **kwargs: Any
    ) -> dict:
        """Gọi chat completion buộc trả JSON, dùng cho các tác vụ trích xuất có cấu trúc."""
        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": json.dumps(user_payload, ensure_ascii=False)},
        ]
        response = self.chat(messages, response_format={"type": "json_object"}, **kwargs)
